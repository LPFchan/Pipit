...
| **Immogen Proximity - Window**   | `B99F8D62-A1C3-4E8B-9D2F-5C3A1B4E6D7A` | **100 ms** | Broadcasts for 30 seconds after a fob triple-press to signal that unauthenticated management connections are allowed. High frequency for instant Pipit detection. |
...
| `RENAME:<own slot>`                 | ✓              | ✗                | ✗                                                              |
| `RENAME:<other slot>`               | ✓              | ✗                | ✗                                                              |
| `RECOVER:<slot>:<key>:<ctr>:[name]` | ✗              | ✗                | ✓ *(Only if vehicle is UNLOCKED and Provisioning Window open)* |
| `SETPIN`                            | serial-only    | serial-only      | serial-only                                                    |
| `RESETLOCK`                         | serial-only    | serial-only      | serial-only                                                    |


*(Note: The `RECOVER` command allows an unidentified phone to replace a key, but Guillemot strictly enforces that the vehicle's hardware latch must be HIGH/Unlocked AND the Fob-Authorized Provisioning Window must be open for it to succeed. See Section 7.3).*
...
| `RECOVER:<slot>:<key>:<ctr>:[name]`  | Yes          | Yes                      | Unidentified (requires Provisioning Window open) |
...
Used when a phone is lost or destroyed. The user must be physically near the vehicle and have the Uguisu hardware fob.

1. The user installs Pipit on a new phone and approaches the vehicle.
2. The user triple-presses the Uguisu fob button to open the 30-second Fob-Authorized Provisioning Window (Section 7.4).
3. Pipit connects to Guillemot. Because the window is open, Guillemot accepts the connection without requiring the BLE Pairing PIN.
4. Pipit queries the slots (`SLOTS?`), and Guillemot returns a JSON array.
5. Pipit displays a UI: *"Which device did you lose?"* listing the names (e.g., "Jamie's iPhone").
6. The user selects "Jamie's iPhone" (Slot 1).
7. Pipit generates a random 16-byte AES key using the platform's secure random generator (`SecRandomCopyBytes` on iOS, `SecureRandom` on Android).
8. Pipit issues `RECOVER:1:<new_key>:0:Jamie's iPhone`. Guillemot overwrites the old key, instantly locking out the stolen phone, and establishes the new key and counter.
...
- **Trigger:** Whenever the user presses the Uguisu button **three times in quick succession**, Uguisu broadcasts a special `Window` payload. When Guillemot successfully receives and parses this valid `Window` payload from Slot 0 (Uguisu), it starts or resets a **30-second Provisioning Window** and plays a distinct chime to indicate the window is open.
- **Rationale:** We use a specific triple-press combination rather than opening the window on every unlock to prevent the security threat of exposing the provisioning window during normal daily use.
*   **Behavior During Window:**
    ...
    *   **Timer Pause:** The 30-second countdown is paused the moment a GATT connection is established and resumes if the connection drops. This gives the user ample time to navigate the UI and enter PINs without the window timing out.
- **Condition:** This window is utilized when a user needs to recover a key from a lost phone, or when Slot 1 (Owner Phone) is empty, allowing the user to seamlessly opt-in to the Phone Key ecosystem wirelessly even months after the immobilizer was physically installed in the vehicle.
...
- **Stateful Beaconing:** Implement advertising logic to dynamically swap the Service UUID between `Locked`, `Unlocked`, and `Window Open` based on the internal state machine.
...
- **Security Manager (SMP):** Configure the SoftDevice security manager to require an Authenticated Link for the Management characteristics. Feed the plaintext 6-digit PIN from flash into the `ble_gap_opt_passkey_t` structure. Dynamically bypass this requirement when the 30-second Provisioning Window is active (except for Owner slot commands like `RECOVER:1`).
...
- **Parser Expansion:** Update the serial parser to handle the new `RENAME` command, accept names in the `PROV` command, format `SLOTS?` output as JSON, accept 6 plaintext digits for `SETPIN`, and process the `RECOVER` command. Route incoming GATT `Management Command` writes into this same parser. The parser must gate `SETPIN` and `RESETLOCK` to serial-only (or Provisioning Window); reject these commands when the source is a standard GATT connection (see Section 6.1).
- **Provisioning Window Logic:** Implement the 30-second timer, pausing logic during active GATT connections, and special `Window` payload parser. Implement contextual privilege elevation: if Slot 1 is empty, allow `SETPIN` and `PROV:1`. If Slot 1 is occupied, only allow `RECOVER`, and mandate that `RECOVER:1` requires the correct PIN (either via SMP bonding or a `SETPIN` verification).
- **Audio Feedback / Buzzer Control:** Add GPIO logic to trigger a distinct sequence (e.g., three fast beeps at 4 kHz) specifically tied to the successful validation of the `Window` payload, confirming the window is open to the user.
...
- **Triple-Press Detection:** Implement a button debounce and timing mechanism to detect three distinct presses within a short window (~1.5 seconds).
- **Window Payload:** When a triple-press is detected, format the payload using the standard `0x494D` prefix but with a distinct encrypted command byte (e.g., `0x04 = Window`) so Guillemot can recognize the intent and subsequently change its own advertised Service UUID.
...
- **"recover key from lost phone >"** — a subtle text link at the bottom of the screen. Tapping it launches the Recover Key from Lost Phone Flow (Section 10.8).
...
- `window flash` → emissive color: `#0000FF`, same intensity ramp as unlock (plays three times in quick succession)

*(Note: Triple-pressing the digital fob in Pipit will NOT open the provisioning window. For security, only the physical Uguisu fob can authorize this window. If a user triple-presses the digital fob, it will play an Easter egg animation—e.g., the 3D model spinning rapidly or the LED flashing a rainbow sequence.)*
...
| **Triple press registered (Easter egg)**           | `.rigid` impact (fires three times)                          | `HapticFeedbackConstants.GESTURE_END` or triple short `VibrationEffect`              |
...
- **Easter egg chime:** A playful ascending trill (~200 ms) that plays when the digital fob is triple-pressed.
...
2. **Keys (Inline Key Management)**
  ...
  2. Pipit then automatically sends the same `PROV` command to Guillemot via the BLE Management Command characteristic to sync the vehicle with the new fob. The old fob is immediately locked out.
    Rename or Delete options. The hardware fob's name is always "Uguisu" and Slot 0 cannot be vacated.
    owner's own slot):** Shows the slot number and device name. **No three-dot menu** — the owner cannot replace or delete their own slot from here. Use "Transfer to New Phone" in the DEVICE section to migrate.
    uest slots (2–3):** Show the slot number and device name. A **three-dot menu button (⋮)** with:
    me** — Opens a text input dialog (max 24 characters). On confirm, sends `RENAME:<slot>:<new_name>`.
    ace** — Launches the Replace Flow (Section 10.7). Revokes this slot and provisions a new phone in its place.
    te** — Confirmation dialog: *"Revoke **? This device will be permanently locked out."* On confirm, sends `REVOKE:<slot>`. The row transitions to an empty-slot state.
    est slots (2–3):** Show only a centered **⊕** icon. Tapping the row launches the Provision Guest Phone flow (Section 10.6).
    ot 1:** Should not normally occur (Slot 1 is provisioned during initial Whimbrel setup). If it somehow appears empty, the ⊕ icon is not shown — Slot 1 can only be provisioned via Whimbrel.
3. **Device**
...
### 10.8 Recover Key from Lost Phone Flow

Launched from the Onboarding camera screen by tapping **"recover key from lost phone >"**. When a phone is lost or broken, the user can recover their key onto a new phone without needing the old phone, by proving physical access to the vehicle via an already-authorized key (the Uguisu fob or a guest phone).

**Step 1 — Physical Authorization Prompt**

```
┌─────────────────────────────┐
│                             │
│  ← Back    Add a Phone Key  │
│                             │
│                             │
│  ❶ Get near your vehicle.   │
│                             │
│  ❷ Press the button three   │
│    times on your Uguisu     │
│    fob.                     │
│                             │
│       Scanning...           │
│                             │
...
**Step 2 — Connection and Recovery**
There is no manual "Connect" button. Instead, Pipit's BLE scanner is actively looking for Guillemot's "Window Open" Service UUID.

1. When the user triple-presses the Uguisu fob, Guillemot changes its advertised UUID to the `Window Open` state.
2. Pipit instantly detects this UUID, vibrates the phone (haptic `.success`), automatically connects to Guillemot, and the 30-second timer on Guillemot is paused.
3. Pipit queries `SLOTS?` and displays the active phone slots for the user to choose which device to recover (e.g., "Owner's Phone", "Guest 1").

**Step 3 — Key Generation and Storage**

1. The user selects a slot. 
   - If they select an **Owner** slot (`Slot 1`), the OS prompts for the 6-digit BLE PIN to prevent guest hijacking. 
   - If they select a **Guest** slot (`Slot 2` or `3`), no PIN is required.
2. Pipit automatically generates a 16-byte AES key and executes the `RECOVER:<slot>:<new_key>:0:<name>` command (as per Section 7.3).
3. The new key is stored directly in the local secure keystore (no QR needed — the key is for this phone).
4. Pipit proceeds directly to the onboarding Done screen (Step 5 of the Onboarding Flow).

---

### 10.10 Migration Flow (Transfer to New Phone)
...
### 10.12 USB Flashing (Android Only)
...
**10.12.2 Flash Uguisu (CDC Serial)**
...
### 10.13 State Diagram — Home Screen
...
### 10.14 Error & Edge Case Handling
