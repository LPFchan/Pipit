...
- Treat [PIPIT_MASTER_ARCHITECTURE.md](../PIPIT_MASTER_ARCHITECTURE.md) as the source of truth for slot semantics, BLE flows, provisioning, and security constraints.
- Keep protocol, crypto, counter, and shared business logic in `shared/`; keep OS integration in the native app targets.
- iOS-specific UI, BLE, CoreLocation, and RealityKit work lives under `iosApp/iosApp/`.
...
- [iosApp/iosApp/UI/OnboardingView.swift](../iosApp/iosApp/UI/OnboardingView.swift) and [iosApp/iosApp/UI/SettingsView.swift](../iosApp/iosApp/UI/SettingsView.swift) for current iOS UI patterns.
